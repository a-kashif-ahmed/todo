  ...
  return (
    <div className="w-72 border-l border-border bg-surface-2 h-full overflow-y-auto">
      ...
      <div className="p-4 space-y-5">
        ...
        {/* System health */}
        <div>
          <p className="text-[11px] text-gray-500 uppercase tracking-wide mb-2">System Health</p>
          <div className="flex items-center justify-between bg-surface rounded-lg border border-border px-3 py-2.5">
            ...
            {latencyMs !== undefined && (
              <span className="text-xs text-gray-500">{latencyMs}ms avg</span>
            )}
          </div>
        </div>
        ...
        {/* Dependencies */}
        <div>
          <p className="text-[11px] text-gray-500 uppercase tracking-wide mb-2">Upstream Dependencies</p>
        ...
        {/* Recent changes */}
        <div>
          <p className="text-[11px] text-gray-500 uppercase tracking-wide mb-2">Recent Changes</p>
          <div className="space-y-2">
            {recentChanges.length === 0 && (
              <span className="text-xs text-gray-500">No recent changes</span>
            )}
            {recentChanges.map((c, i) => (
              <div key={i} className="text-xs">
                <p className="text-gray-300">{c.label}</p>
                <p className="text-gray-500 text-[11px]">{c.time}</p>
              </div>
            ))}
          </div>
        </div>
