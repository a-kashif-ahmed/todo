    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-80 flex-shrink-0 border-r border-border flex flex-col">
        ...
        <div className="p-4 border-b border-border space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-surface-2 border border-border rounded-xl p-3.5">
              <p className="text-[10px] text-gray-500 uppercase mb-1">Avg. Time to Resolve</p>
            ...
            <div className="bg-surface-2 border border-border rounded-xl p-3.5">
              <p className="text-[10px] text-gray-500 uppercase mb-1">AI Confidence Score</p>
              <div className="flex items-center justify-between">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-3 py-2 text-[10px] text-gray-500 uppercase font-semibold tracking-wide border-b border-border">
            Last 24h
          </div>
          {loading ? (
            ...
            incidents.map(inc => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-semibold text-gray-500">{timeAgo(inc.detected_at)}</p>
                  <AlertTriangle size={12} className="text-status-error" />
                </div>
                <p className="text-sm font-medium text-white truncate">{workflows[inc.workflow_id] || "Unknown workflow"}</p>
                <p className="text-xs text-gray-500 mt-0.5 truncate">{inc.error_message || "Investigating..."}</p>
              </button>
            ))
          )}
        </div>
        ...
        {selected ? (
          <div className="p-8">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-xl font-semibold text-white">{workflows[selected.workflow_id] || "Unknown"}</h2>
                <p className="text-sm text-gray-500 mt-0.5">{timeAgo(selected.detected_at)}</p>
              </div>
              <button
                onClick={() => router.push(`/workflows/${selected.workflow_id}/incidents/${selected.id}`)}
                className="bg-brand-blue text-white text-sm font-medium rounded-lg px-4 py-2 hover:opacity-90 transition-opacity"
              >
                Full Analysis →
              </button>
            </div>
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center max-w-sm">
              ...
              <h3 className="text-xl font-semibold text-white mb-2">No active investigation</h3>
              ...
              <div className="flex items-center justify-center gap-2 bg-surface-2 border border-border rounded-lg px-4 py-2.5 text-xs text-gray-500">
                <span>Press</span>
